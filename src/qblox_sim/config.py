...
class SimulationConfig:
    qubits: Dict[str, QubitConfig] = field(default_factory=lambda: {"q0": QubitConfig()})
    resonators: Dict[str, ResonatorConfig] = field(default_factory=lambda: {"q0": ResonatorConfig()})
    ...
    def resonator(self) -> ResonatorConfig:
        ...
        return self.resonators.get("q0", next(iter(self.resonators.values()), ResonatorConfig()))

    @classmethod
    def from_dict(cls, params: dict[str, Any]) -> "SimulationConfig":
        """Parses a multi-qubit topology configuration dictionary."""
        
        qubits = {k: QubitConfig(**v) if isinstance(v, dict) else v for k, v in params.get("qubits", {}).items()}
        resonators = {k: ResonatorConfig(**v) if isinstance(v, dict) else v for k, v in params.get("resonators", {}).items()}
        couplings = [CouplingConfig(**c) if isinstance(c, dict) else c for c in params.get("couplings", [])]
        acq = AcquisitionConfig(**params["acquisition"]) if "acquisition" in params else AcquisitionConfig()
