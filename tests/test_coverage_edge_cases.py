...
def test_thresholded_acquisition_with_rotation_and_threshold():
    ...
    res = handler.process(
        state=state, t_list=np.linspace(0, 100e-9, 10), states=[state] * 10,
        acq_time=10e-9, acq_duration=50e-9, acq_info=acq_info,
        cfg=cfg, a_op=a_op, ad_op=a_op.dag()
    )
...
def test_trace_acquisition_fallback_when_empty_indices():
    ...
    res = handler.process(
        state=state, t_list=np.linspace(0, 10e-9, 10), states=[state] * 10,
        acq_time=100e-9, acq_duration=50e-9, acq_info={},
        cfg=cfg, a_op=a_op, ad_op=a_op.dag()
    )
...
def test_signals_drag_pulse_and_edge_cases():
    ...
    drag_pulse = [{
        'port': 'q0:mw', 'abs_time': 10e-9, 'duration': 40e-9,
        'amp': 0.5, 'phase': 45.0, 'wf_func': 'drag', 'beta': 0.1, 'sigma': 10e-9
    }]
    ...
    out_of_bounds_pulse = [{'port': 'q0:mw', 'abs_time': 500e-9, 'duration': 10e-9, 'amp': 0.5}]
...
def test_engine_quadrature_drives():
    cfg = SimulationConfig(
        qubits={'q0': QubitConfig(N_q=2)},
        resonators={'q0': ResonatorConfig(N_res=2)}
    )
    system = QuantumSystem(cfg)
    engine = QuTiPEngine()
    ...
    res = engine.run(system=system, drives=drives, t_list=t_list, initial_state=init_state)
...
def test_simulation_result_expectation_exceptions():
    ...
    res_sys = SimulationResult(states=[], t_list=np.array([]), measurements=[], system=sys)
...
def test_uncompiled_schedule_fallback_compiler(default_qubit_params):
    ...
    with patch.object(Schedule, "timing_table", new_callable=PropertyMock, side_effect=AttributeError("Not compiled")):
...
def test_q1simulator_solver_failure_exception(default_qubit_params):
    ...
    with QbloxQ1Simulator(default_qubit_params) as sim:
        ...
        sim.engine.run = MagicMock(side_effect=RuntimeError("Solver Convergence Failed"))
