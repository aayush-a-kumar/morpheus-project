...
def test_flux_drive_phase_accumulation():
    ...
    cfg = SimulationConfig(
        qubits={"q0": QubitConfig(f_q=5e9, f_d=5e9, flux_freq_per_volt=25e6)},
        dt=1e-9
    )
    ...
    excited = qutip.tensor(qutip.basis(cfg.qubits["q0"].N_q, 1), qutip.basis(cfg.resonators["q0"].N_res, 0))
    ...
    assert np.isclose(fidelity, expected_fidelity, atol=1e-3), f"Flux drive failed to accumulate correct phase! Fidelity: {fidelity}"
