...
def test_qutip_engine_basic_execution(default_system):
    ...
    result = engine.run(
        system=default_system, drives=drives, t_list=t_list, initial_state=initial_state
    )
    ...
    rho_q = (
        final_state.ptrace(0)
        if final_state.type == "oper"
        else qutip.ket2dm(final_state).ptrace(0)
    )
...
def test_multi_qubit_drive_routing():
    ...
    cfg = SimulationConfig(
        qubits={"q0": QubitConfig(N_q=2), "q1": QubitConfig(N_q=2)}, resonators={}
    )
    ...
    result = engine.run(
        system=system, drives=drives, t_list=t_list, initial_state=initial_state
    )
