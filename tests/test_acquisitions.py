...
from qblox_sim.simulator import QbloxQ1Simulator, QbloxQutipSimulator
...
def test_ssb_integration_iq_acquisition(default_qubit_params):
    ...
    with QbloxQ1Simulator(default_qubit_params) as sim:
        ...
        assert len(measurements) > 0, (
            "Failed to retrieve any measurements from simulator."
        )
        m = measurements[0]

        assert "I" in m and "Q" in m, (
            "SSBIntegrationComplex measurement missing 'I' or 'Q' voltage outputs."
        )
        assert "prob_0" in m and "prob_1" in m, (
            "Measurement output missing qubit population probabilities."
        )

        # 2. Physics & Value Bounds Check
        assert np.isclose(m["prob_0"], 1.0, atol=1e-2), (
            f"Expected ground state prob_0 ≈ 1.0, got {m['prob_0']:.4f}"
        )
        assert np.isfinite(m["I"]) and np.isfinite(m["Q"])
        assert np.isclose(m["I"], 0.05, atol=0.1), (
            f"I voltage {m['I']} deviated excessively from V_0 centroid"
        )
        assert np.isclose(m["Q"], 0.05, atol=0.1), (
            f"Q voltage {m['Q']} deviated excessively from V_0 centroid"
        )
...
def test_thresholded_acquisition(default_qubit_params):
    ...
    excited_state = qutip.tensor(qutip.basis(n_q, 1), qutip.basis(n_res, 0))
    ...
    assert len(measurements) > 0, (
        "No measurements returned for thresholded acquisition."
    )
    m = measurements[0]

    assert "outcome" in m, "ThresholdedAcquisition missing discrete 'outcome' field."
    assert m["outcome"] in (0, 1), (
        f"Discrete outcome must be 0 or 1, got {m['outcome']}"
    )
    assert np.isclose(m["prob_1"], 1.0, atol=1e-2), (
        f"Expected excited state prob_1 ≈ 1.0, got {m['prob_1']:.4f}"
    )
...
def test_q1simulator_mock_memory_injection(default_qubit_params):
    ...
    with QbloxQ1Simulator(default_qubit_params) as sim:
        ...
        assert sim.readout_mod in connected, (
            f"Readout module {sim.readout_mod} not found in connected cluster."
        )
...
def test_joint_probabilities_2qubit_bell_state():
    ...
    cfg = SimulationConfig(
        qubits={"q0": QubitConfig(N_q=3), "q1": QubitConfig(N_q=3)},
...
def test_joint_probabilities_leakage_detection():
    ...
    cfg = SimulationConfig(qubits={"q0": QubitConfig(N_q=3), "q1": QubitConfig(N_q=3)})
    handler = SSBIntegrationHandler()
...
def test_3qubit_dynamic_joint_probabilities():
    ...
    expected_keys = {
        f"prob_{b0}{b1}{b2}" for b0 in (0, 1) for b1 in (0, 1) for b2 in (0, 1)
    }
